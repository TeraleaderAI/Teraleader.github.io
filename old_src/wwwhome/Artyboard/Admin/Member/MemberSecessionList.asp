<%
	DIM intTopMenu, intLeftMenu, isAdminMenu, isAdminPopup, strAdminPrevUrl
	intTopMenu   = 4
	intLeftMenu  = 8
	isAdminMenu  = 2
	isAdminPopup = False
%>
<!-- #include file = "Head.asp" -->
<%
	DIM intPage, intPageSize, intTotalCount, intPageCount
	intPage     = REQUEST.QueryString("intPage") : IF intPage     = "" THEN intPage     = 1
	intPageSize = 20

	SET RS = DBCON.EXECUTE("SELECT COUNT([intNum]) AS [RecCount] FROM [MPLUS_MEMBER_LIST] WHERE [strAdmin] != '2' AND [bitSecession] = '1' ")

	intTotalCount = RS(0)
	intPageCount = INT((intTotalCount - 1) / intPageSize) + 1

	SET RS = DBCON.EXECUTE("SELECT TOP " & intPageSize & " [intNum], [strLoginID], [strLoginName], [intPoint] = (SELECT SUM([moneyPoint]) FROM [MPLUS_MEMBER_POINT] WHERE [strLoginID] = [MPLUS_MEMBER_LIST].[strLoginID]), [intBoardCount], [intCommentCount], [intVote], [intVisit], [dateSignDate], [dateRegDate], [bitAuth], [strSecessionMemo], [intLevel] = (SELECT [intLevel] FROM [MPLUS_GROUP] WHERE [strGroupCode] = [MPLUS_MEMBER_LIST].[strGroup]) FROM [MPLUS_MEMBER_LIST] WHERE [intNum] NOT IN (SELECT TOP " & (intPage - 1) * intPageSize & " [intNum] FROM [MPLUS_MEMBER_LIST] WHERE [strAdmin] != '2' AND [bitSecession] = '1' ORDER BY [intNum] DESC) AND [strAdmin] != '2' AND [bitSecession] = '1' ORDER BY [intNum] DESC ")
%>
						<table width="750" border="0" cellspacing="0" cellpadding="0">
							<form name="theForm" method="post">
              <tr>
                <td>
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td height="35"><img src="../images/main_title12.gif" width="121" height="19"></td>
                      <td align="right">관리자 홈 &gt; 회원관리 &gt; <b>탈퇴회원 리스트</b></td>
                    </tr>
                  </table>                </td>
              </tr>
              <tr>
                <td>&nbsp;</td>
              </tr>
              <tr>
                <td height="30"><img src="../images/stit_icon.gif" width="6" height="11"> <span style="font-size:14px"><strong>탈퇴요청 회원정보</strong></span></td>
              </tr>
							<tr>
								<td height="3" bgcolor="#CCCCCC"></td>
							</tr>
              <tr>
                <td style="padding-top:5; padding-bottom:5;">전체 : <font color="#C6325B"><b><%=intTotalCount%></b></font>&nbsp;&nbsp;<%=intPage%>/<%=intPageCount%> 페이지</td>
              </tr>
              <tr>
                <td>
									<table width="100%"  border="0" cellpadding="0" cellspacing="0">
										<tr align="center" bgcolor="EB766F">
											<td colspan="13" class="table_Round1"></td>
										</tr>
										<tr align="center" bgcolor="EB766F">
											<td height="30" class="table_Txt1" nowrap>선택</td>
											<td height="30" class="table_Txt1" nowrap>번호</td>
											<td height="30" class="table_Txt1" nowrap>아이디</td>
											<td height="30" class="table_Txt1" nowrap>이름</td>
											<td class="table_Txt1" nowrap>포인트</td>
											<td class="table_Txt1" nowrap>접속</td>
											<td class="table_Txt1" nowrap>최근접속</td>
											<td class="table_Txt1" nowrap>게시글/댓글/추천</td>
											<td class="table_Txt1" nowrap>가입일</td>
											<td class="table_Txt1" nowrap>등급</td>
											<td width="45" nowrap class="table_Txt1">탈퇴사유</td>
											<td width="45" height="30" nowrap class="table_Txt1">취소</td>
											<td width="45" align="center" class="table_Txt1" nowrap>삭제</td>
										</tr>
										<tr bgcolor="EB766F">
											<td colspan="13" class="table_Round1"></td>
										</tr>
<%
	IF RS.EOF THEN
%>
										<tr align="center" bgcolor="#FFFFFF">
											<td colspan="13" class="table_ListSubText1">등록된 회원이 없습니다.</td>
										</tr>
										<tr>
											<td colspan="13" class="table_ListSubLine1"></td>
										</tr>
<%
	ELSE
		DIM iCount, intMemberNum
		WHILE NOT(RS.EOF)
			iCount = iCount + 1
			intMemberNum = int(intTotalCount) - (INT(intPageSize) * (INT(intPage) - 1)) - iCount + 1
%>
										<tr bgcolor="#FFFFFF" align="center">
											<td class="table_ListSubText1"><input name="strLoginID" type="checkbox" id="strLoginID" value="<%=RS("strLoginID")%>" class="no_Line"></td>
											<td class="table_ListSubText1"><%=intMemberNum%></td>
											<td class="table_ListSubText1"><%=RS("strLoginID")%></td>
											<td class="table_ListSubText1"><%=RS("strLoginName")%></td>
											<td class="table_ListSubText1"><a href="javascript:popupLayer('MemberPoint.asp?strLoginID=<%=RS("strLoginID")%>',810,534);"><%=GetMoneyComma(RS("intPoint"))%></a></td>
											<td class="table_ListSubText1"><%=RS("intVisit")%></td>
											<td class="table_ListSubText1"><%=GetDateType(0, RS("dateSignDate"))%></td>
											<td class="table_ListSubText1"><%=RS("intBoardCount")%> / <%=RS("intCommentCount")%> / <%=RS("intVote")%></td>
											<td class="table_ListSubText1"><%=REPLACE(FORMATDATETIME(RS("dateRegDate"),2), "-", "/")%></td>
											<td class="table_ListSubText1">Lv.<%=RS("intLevel")%></td>
											<td class="table_ListSubText1"><a href="javascript:;" onClick="OnSecessionInfo('<%=intMemberNum%>');return false;"><img src="../images/btn_view_s.gif" width="38" height="16" border="0"></a></td>
											<td class="table_ListSubText1"><a href="javascript:;" onclick="OnUnSecession('<%=RS("strLoginID")%>');return false;"><img src="../images/btn_cancel_s.gif" width="38" height="16" border="0" /></a></td>
											<td class="table_ListSubText1"><a href="javascript:;" onclick="OnMemberRemove('<%=RS("strLoginID")%>');return false;"><img src="../images/btn_delete_s.gif" width="38" height="16" border="0" /></a></td>
										</tr>
										<tr>
											<td colspan="13" class="table_ListSubLine1"></td>
										</tr>
										<tr bgcolor="#FFFFFF" align="left" id="strMemberList<%=intMemberNum%>" style="display:none">
											<td colspan="13" bgcolor="#FFFFFF" class="table_ListSubText1" style="padding:5 5 5 5;"><%=GetReplaceTag2Text(RS("strSecessionMemo"))%></td>
											</tr>
										<tr id="strMemberList<%=intMemberNum%>" style="display:none">
											<td colspan="13" class="table_ListSubLine1"></td>
										</tr>
<%
		RS.MOVENEXT
		WEND
	END IF
%>
										<tr>
											<td colspan="13" height="1"></td>
										</tr>
										<tr>
											<td colspan="13" class="table_ListSubBLine1"></td>
										</tr>
									</table>								</td>
              </tr>
              <tr>
                <td height="40">
									<table width="100%" border="0" cellspacing="0" cellpadding="0">
										<tr>
											<td><a href="javascript:;" onclick="OnMemberSelect();return false;"><img src="../images/btn_select_all_w.gif" width="102" height="19" border="0"></a></td>
											<td align="right">
											<a href="javascript:;" onclick="OnSelectRemove();return false;"><img src="../images/btn_select_member_delete_w.gif" width="116" height="19" border="0"></a>
											<a href="javascript:;" onclick="OnSelectUnSecession();return false;"><img src="../images/btn_select_member_cancel2_w.gif" width="117" height="19" border="0"></a>
											</td>
										</tr>
									</table>
								</td>
              </tr>
              <tr>
                <td height="40" align="center"><% CALL GotoPageHTML(intPage, intPageCount) %></td>
              </tr>
							<tr>
								<td style="padding:10 10 10 10">
									<fieldset CLASS="infobox">
									<legend CLASS="infotitle">&nbsp;<img src="../images/check.gif" align="absmiddle">&nbsp;</legend>
									<table width="100%"  border="0" cellspacing="10" cellpadding="0">
										<tr>
											<td>
											<LI>탈퇴요청을 한 회원들의 정보를 확인하실 수 있으며, 탈퇴사유 버튼을 클릭해서 사유를 확인하실 수 있습니다.</LI>
											<LI>선택한 회원을 삭제하시면 DB에서 회원정보는 영구적으로 삭제처리 됩니다.</LI>
											<LI>회원가입 관련설정에서 회원탈퇴 방법을 관리자 확인 후 정보삭제로 설정한 경우에만 탈퇴목록이 출력됩니다.</LI>
											</td>
										</tr>
									</table>
									</fieldset>								</td>
							</tr>
              <tr>
                <td>&nbsp;</td>
              </tr>
							</form>
            </table>
<%
	SUB GotoPageHTML(intPage, intPageCount)

		RESPONSE.WRITE "<table border='0' cellpadding='0' cellspacing='0'>" & vbcrlf
		RESPONSE.WRITE "	<tr>" & vbcrlf

		DIM intBlockPage, I
		intBlockPage = INT((intPage - 1) / 10) * 10 + 1

		IF intBlockPage = 1 THEN
		ELSE
    	RESPONSE.WRITE "<td id='mytd'><img src='../images/page_allow1.gif' vspace='2'>&nbsp;<a href=""javascript:;"" OnClick=""OnPageMove('" & intBlockPage - 10 & "','" & topMenu & "','" & leftMenu & "');return false;"" class='brn01'>이전</a></td>"
		END IF

		RESPONSE.WRITE "		<td  width='1' nowrap bgcolor='#cccccc'></td>" & vbcrlf

		i = 1
		
		DO UNTIL i > 10 OR intBlockPage > intPageCount

			RESPONSE.WRITE "		<td id='mytd' onMouseOver=""this.style.background='#f7f7f7'"" onMouseOut=""this.style.background=''"" align=center onClick=""OnPageMove('" & intBlockPage & "','" & topMenu & "','" & leftMenu & "');return false;"">"
		
			IF INT(intBlockPage) = INT(intPage) THEN RESPONSE.WRITE "<font color='#ff7635'><b>" & intBlockPage & "</b></font>" ELSE RESPONSE.WRITE "<b>" & intBlockPage & "</b>"

			RESPONSE.WRITE "</td>" & vbcrlf
			RESPONSE.WRITE "<td  width=1 nowrap bgcolor='#cccccc'></td>" & vbcrlf

			intBlockPage = intBlockPage + 1
			I = I + 1
		
		LOOP

    IF intBlockPage > intPageCount THEN
		ELSE
			RESPONSE.WRITE "<td id='mytd'>&nbsp;<a href=""javascript:;"" OnClick=""OnPageMove('" & intBlockPage & "','" & topMenu & "','" & leftMenu & "');return false;"" class='brn01'>다음</a>&nbsp;<img src='../images/page_allow2.gif' vspace='2'></td>"
		END IF

		RESPONSE.WRITE "	</tr>" & vbcrlf
		RESPONSE.WRITE "</table>" & vbcrlf

	END SUB
%>
<script language="javascript">

	var SET_NOW_PAGE = "<%=intPage%>";
	var SET_intTotalCount = "<%=intTotalCount%>";

	function OnMemberSelect(){
		if (SET_intTotalCount != "0"){

			obj = document.all['strLoginID'];

			if (SET_intTotalCount == "1"){
				if (obj.checked == true){obj.checked = false;}else{obj.checked = true}
			}else{
				var cntBox = obj.length - 1;
				for(var i = 0; i <= cntBox; i++){
					if (obj[i].checked == false){obj[i].checked=true;}else{obj[i].checked=false;}
				}
			}
		}
	}

	function OnPageMove(intPage){
		document.theForm.action = "MemberSecessionList.asp?intPage=" + intPage;
		document.theForm.submit();
	}

	function OnMemberSelectCheck(){
		var sMember = "";
		if (SET_intTotalCount == "0"){
			return false;
		}else{
			obj = document.all['strLoginID'];
			if (SET_intTotalCount == "1"){
				if (obj.checked == true){
					sMember = sMember + obj.value + ",";
				}
			}else{
				var cntBox = obj.length - 1;
				for(var i = 0; i <= cntBox; i++){
					if (obj[i].checked == true){
						sMember = sMember + obj[i].value + ",";
					}
				}
			}
			return sMember;
		}
	}

	function OnSecessionInfo(str){
		if (document.all['strMemberList' + str][0].style.display == "none"){
			document.all['strMemberList' + str][0].style.display = "block";
			document.all['strMemberList' + str][1].style.display = "block";
		}else{
			document.all['strMemberList' + str][0].style.display = "none";
			document.all['strMemberList' + str][1].style.display = "none";
		}
	}

	function OnSelectUnSecession(){

		var sMember = "";
		sMember = OnMemberSelectCheck();
		if (!sMember || sMember == ""){
			alert("회원을 선택해 주시기 바랍니다.");
			return false;
		}

		if (confirm("선택된 회원의 탈퇴신청을 취소하시겠습니까?")){
			document.theForm.action = "MemberSecessionList_ok.asp?Action=selectsecession&intPage=" + SET_NOW_PAGE;
			document.theForm.submit();
		}
	}

	function OnUnSecession(str){
		if (confirm("선택된 회원의 탈퇴신청을 취소하시겠습니까?")){
			document.theForm.action = "MemberSecessionList_ok.asp?Action=secession&strLoginID=" + str + "&intPage=" + SET_NOW_PAGE;
			document.theForm.submit();
		}
	}

	function OnMemberRemove(str){
		if (confirm("선택된 회원을 삭제하시겠습니까?")){
			document.theForm.action = "MemberSecessionList_ok.asp?Action=remove&strLoginID=" + str + "&intPage=" + SET_NOW_PAGE;
			document.theForm.submit();
		}
	}

	function OnSelectRemove(){
		var sMember = "";
		sMember = OnMemberSelectCheck();
		if (!sMember || sMember == ""){
			alert("회원을 선택해 주시기 바랍니다.");
			return false;
		}

		if (confirm("선택된 회원을 삭제하시겠습니까?")){
			document.theForm.action = "MemberSecessionList_ok.asp?Action=selectremove&intPage=" + SET_NOW_PAGE;
			document.theForm.submit();
		}
	}

</script>
<!-- #include file = "Foot.asp" -->