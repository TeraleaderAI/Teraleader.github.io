...
<%
	DIM strGroupCode, intPage, intPageSize, intTotalCount, intPageCount, Query1, Query2, I
	...
	SET RS = DBCON.EXECUTE("SELECT COUNT([intNum]) AS [RecCount] FROM [MPLUS_BOARD_GROUP_MEMBER] WHERE [strGroupCode] = '" & strGroupCode & "' ")
...
<%
	SET RS = DBCON.EXECUTE("SELECT TOP " & intPageSize & " [b].[strLoginID], [b].[strLoginName], [intLevel] = (SELECT [intLevel] FROM [MPLUS_GROUP] WHERE [strGroupCode] = [b].[strGroup]), [b].[dateRegDate] FROM [MPLUS_BOARD_GROUP_MEMBER][a] INNER JOIN [MPLUS_MEMBER_LIST][b] ON [a].[strLoginID] = [b].[strLoginID] WHERE [a].[intNum] NOT IN (SELECT TOP " & (intPage - 1) * intPageSize & " [intNum] FROM [MPLUS_BOARD_GROUP_MEMBER] WHERE [strGroupCode] = '" & strGroupCode & "' ORDER BY [intNum] DESC) AND [a].[strGroupCode] = '" & strGroupCode & "' ORDER BY [a].[intNum] DESC ")
...
<script language="javascript">
	...
	function OnMemberRemove(str){
		if(confirm("선택한 회원을 삭제하시겠습니까?")){
			document.theForm.action = "GroupMember_ok.asp?Action=remove&strGroupCode=" + SET_strGroupCode + "&strLoginID=" + str;
	...
	function OnMemberSelectRemove(){
		...
		if(confirm("선택된 회원을 삭제하시겠습니까?")){
			document.theForm.action = "GroupMember_ok.asp?Action=sremove&strGroupCode=" + SET_strGroupCode;
			document.theForm.submit();
		}
	...
	function OnPageMove(intPage){
		...
		document.theForm.action = "GroupMemberList.asp?strGroupCode=" + SET_strGroupCode + "&intPage=" + intPage;
		document.theForm.submit();
