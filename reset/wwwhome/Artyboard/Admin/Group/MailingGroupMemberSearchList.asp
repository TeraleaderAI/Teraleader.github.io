...
<%
	DIM strCode, intPage, intPageSize, intTotalCount, intPageCount, Query, strGroup
	...
	strSearchKey     = REQUEST.Form("strSearchKey")
	strSearchKeyWord = REQUEST.Form("strSearchKeyWord")
	strGroup         = REQUEST.FORM("strGroup")

	IF strGroup <> "" THEN Query = " AND [strGroup] = '" & strGroup & "' "
	IF strSearchKeyWord <>"" THEN Query = " AND " & strSearchKey & " LIKE '%" & strSearchKeyWord & "%' "
	
	SET RS = DBCON.EXECUTE("SELECT COUNT([intNum]) AS [RecCount] FROM [MPLUS_MEMBER_LIST] WHERE [strLoginID] NOT IN (SELECT [strLoginID] FROM [MPLUS_MEMBER_MAILING_MEMBER] WHERE [strGroup] = '" & strCode & "' ) " & Query)
...
<form name="theForm" method="post" action="MailingGroupMemberSearchList.asp?strCode=<%=strCode%>" onSubmit="return OnMemberSearch();">
	<tr>
...
<%
	SET RS = DBCON.EXECUTE("SELECT TOP " & intPageSize & " [strLoginID], [strLoginName], [intLevel] = (SELECT [intLevel] FROM [MPLUS_GROUP] WHERE [strGroupCode] = [MPLUS_MEMBER_LIST].[strGroup]), [dateRegDate] FROM [MPLUS_MEMBER_LIST] WHERE [intNum] NOT IN (SELECT TOP " & (intPage - 1) * intPageSize & " [intNum] FROM [MPLUS_MEMBER_LIST] WHERE [strLoginID] NOT IN (SELECT [strLoginID] FROM [MPLUS_MEMBER_MAILING_MEMBER] WHERE [strGroup] = '" & strCode & "') " & Query & " ORDER BY [intNum] DESC) AND [strLoginID] NOT IN (SELECT [strLoginID] FROM [MPLUS_MEMBER_MAILING_MEMBER] WHERE [strGroup] = '" & strCode & "') " & Query & " ORDER BY [intNum] DESC ")
...
<%
	...
	WHILE NOT(RS.EOF)
		RESPONSE.WRITE "<option value='" & RS("strGroupCode") & "'"
		IF strGroup = RS("strGroupCode") THEN RESPONSE.WRITE " SELECTED"
		RESPONSE.WRITE ">" & RS("strGroupName") & "</option>" & vbcrlf
	RS.MOVENEXT
	WEND
%>
    </select>
		...
		<input name="strSearchKeyWord" type="text" value="<%=strSearchKeyWord%>" size="20">
		<input type="image" src="../images/btn_member_search_w.gif" border="0" align="absmiddle" class="no_Line">
		<img src="../images/btn_search_cancel_w.gif" width="68" height="19" align="absmiddle"></td>
	</tr>
	<tr>
...
<script language="javascript">
	...
	function OnMemberAdd(str){
		if (confirm("선택한 회원을 등록하시겠습니까?")){
			document.theForm.action = "MailingGroupMember_ok.asp?Action=add&strCode=" + SET_strGroupCode + "&strLoginID=" + str;
	...
	function OnMemberSelectAdd(){
		...
		if(confirm("선택된 회원을 등록하시겠습니까?")){
			document.theForm.action = "MailingGroupMember_ok.asp?Action=sAdd&strCode=" + SET_strGroupCode;
			document.theForm.submit();
		}
	...
	function OnMemberSearch(){
		...
		if (str.value == "" && document.all['strGroup'].value == ""){alert("검색하실 단어 또는 그룹을 선택해 주시기 바랍니다.");str.focus();return false;}
	}

	function OnPageMove(intPage){

		document.theForm.action = "MailingGroupMemberSearchList.asp?strCode=" + SET_strGroupCode + "&intPage=" + intPage;
		document.theForm.submit();
