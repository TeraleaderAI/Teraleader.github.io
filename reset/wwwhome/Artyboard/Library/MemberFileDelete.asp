...
<%
	...
	SET RS = DBCON.EXECUTE("EXEC [MPLUS_GET_MEMBER_READ] '" & SESSION("strLoginID") & "', '0' ")

	SELECT CASE GetReplaceInput(REQUEST.QueryString("sType"), "S")
	CASE "1"
		IF RS("strPhotoFile") <> "" AND ISNULL(RS("strPhotoFile")) = False THEN
			DBCON.EXECUTE("UPDATE [MPLUS_MEMBER_LIST] SET [strPhotoFile] = '' WHERE [strLoginID] = '" & SESSION("strLoginID") & "' ")
			CALL ExecFileDelete(rootPath & "Pds\Member\Photo\", RS("strPhotoFile"))
		END IF
	CASE "2"
		IF RS("strNameFile") <> "" AND ISNULL(RS("strNameFile")) = False THEN
			DBCON.EXECUTE("UPDATE [MPLUS_MEMBER_LIST] SET [strNameFile] = '' WHERE [strLoginID] = '" & SESSION("strLoginID") & "' ")
