...
function init(){ 
        ...
        document.all['SWATCHES'].onmousedown=swatchSelect; 
				document.all['text'].value = parent.document.all['<%=REQUEST("target")%>'].value;
        obj=document.all['SELECT']; 
				obj.style.backgroundColor = parent.document.all['<%=REQUEST("target")%>'].value;
} 
function test(){ 
...
function apply_color() {
	var color = document.all['text'].value;
	if (color != 0){
		parent.document.all['<%=REQUEST("target")%>'].value = "#" + color;
		parent.document.all['<%=REQUEST("target")%>Prev'].style.backgroundColor = "#" + color;
		parent.closeLayer();
	}
}
...
.outset { BORDER-RIGHT: #292929 1px solid; BORDER-TOP: #69bbe7 1px solid; BORDER-LEFT: #69bbe7 1px solid; BORDER-BOTTOM: #292929 1px solid } 
	.button_new	       { border-left:1px solid #dddddd; border-top:1px solid #dddddd;
											 ...
											 background:#EFEFEF; font-size:9pt; font-family:굴림; padding-top:3px; color:#333333; height:20; cursor:h}
</STYLE> 
</HEAD> 
<BODY onload=init()> 
<DIV id=SUBJECT> 
  ...
  <tr>
    <td align=center>
      <table width="95%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td background="http://enamoofree.godo.co.kr/shop/admin/img/pop_titlebg.gif" align=center><img src="images/colorchart.gif" alt="색상표" width="381" height="49" border="0" align="absmiddle"></td>
        </tr>
      </table>
    </td>
  </tr>
...
<TABLE height="100%" width=96% cellSpacing=0 cellPadding=0 border=0> 
  ...
  <tr>
  <td class=text1>
  <DIV class=inset id=SAMPLE style="position:absolute; width:90px; height:20px; left:5px; top:140px; z-index:2">
  <FONT color=white>현재 색깔</FONT></DIV> 
<DIV class=inset id=SELECT style="position:absolute; width:90px; height:20px; left:5px; top:60px; z-index:2"><FONT color=white>선택된 색깔</FONT></DIV> 
</td>
</tr>
  <TR> 
    <TD valign=top class=text1>선택:<INPUT  size=7 value=0 name=text></TD> 
    
    </TR> 
  <INPUT type=hidden class=inset value=0 name=background> 
  <TR> 
    <TD valign=top class=text1>현재:<INPUT  size=7 value=0 name=current></TD></TR> 
...
<DIV id=CLOSE>
			<table align="center" cellpadding="2" cellspacing="1" width="100%" border="0">
				<tr>
					<td class='tablebgcolor' align="center" height="25">
						<input type="button" value="적 용" onClick="apply_color();" onFocus="this.blur()" class="button_new" style="width:60px">
						<input type="button" value="취 소" onClick="parent.closeLayer();" onFocus="this.blur()" class="button_new" style="width:60px">
					</td>
				</tr>
				<tr>
					<td height="1" class="borderline1"></td>
				</tr>
			</table>
</DIV>
</BODY>
