...
<%
	...
	SET RS = DBCON.EXECUTE("EXEC [MPLUS_GET_MYMENU_COMMENT] '" & strUserID & "', 'C', '" & intPageSize & "' ")
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td height="30">
									<table width="100%" border="0" cellspacing="0" cellpadding="0">
										<tr>
											<td><img src="MyMenu/images/ico_dot.gif" width="9" height="9">댓글&nbsp;<b><%=intTotalCount%>개</b></td>
...
<%
	SET RS = DBCON.EXECUTE("EXEC [MPLUS_GET_MYMENU_COMMENT] '" & strUserID & "', 'L', '" & intPageSize & "', '" & intPage & "' ")

	IF RS.EOF THEN
%>
										<tr>
											<td height="30" colspan="3" align="center" class="board1_txt">등록된 댓글이 없습니다.</td>
										</tr>
<%
	ELSE
...
%>
										<tr>
											<td height="30" align="center" class="board1_text"><%=intNumber%></td>
											<td height="25" class="board1_txt"><a href="Mboard.asp?action=view&strBoardID=<%=RS("strBoardID")%>&intSeq=<%=RS("intBoardSeq")%>" target="_blank"><%=GetCutSubject(GetReplaceTag2Text(StripTags(RS("strContent"))),60)%></a><% IF GetNewBoardTime(48, RS("dateRegDate")) = True THEN %>&nbsp;<img src="MyMenu/MyMenu/images/ico_new.gif" width="10" height="9" align="absmiddle"><% END IF %></td>
...
<script language="javascript">
	...
	function OnPageMove(intPage, strUserID){
		location.href = "MyMenu.asp?Action=CmtList&strUserID=" + strUserID + "&intPage=" + intPage;
	}
