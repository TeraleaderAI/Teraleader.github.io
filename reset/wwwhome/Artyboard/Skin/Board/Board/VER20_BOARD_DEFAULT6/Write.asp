...
<input type="hidden" name="strSessionID" value="<%=SESSION.SESSIONID%>">
	<tr>
		...
		<% END IF %><% IF SESSION("strLoginID") = "" THEN %>
		<a href="<%=CONF_strLoginLink%>" class="skinList">로그인</a>
...
<% IF CONF_bitSubjectStyleLevel = True THEN %>
													<tr>
														...
														<%=GetSubjectStyleForm("4", WRITE_strSubjectStyle(3))%>														</td>
													</tr>
<% END IF %>
												</table>											</td>
										</tr>
										<tr>
											...
											<input name="bitCook" type="checkbox" class="no_Line" id="bitCook" value="1" checked><LABEL STYLE='cursor:hand;' FOR=bitCook>등록자 정보기억</LABEL>											</td>
										</tr>
									</table>								</td>
							</tr>
							<tr>
								<td><textarea name="strContent" cols="70" rows="16" id="strContent" style="display:<% IF CONF_bitUseEditor = False THEN %>block<% ELSE %>none<% END IF %>; width:98%"><%=WRITE_strContent%></textarea>
